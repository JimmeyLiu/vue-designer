	...
	>
		...
		<a-form-item label="展示形式">
			<a-radio-group v-model:value="formState.type" @change="onChange">
				<a-radio-button value="default">普通</a-radio-button>
		...
		<a-collapse v-model:activeKey="activeKey" expandIconPosition="right">
			<a-collapse-panel key="1" header="选项">
				...
				<a-table
					v-if="formState.optionType === 'custom'"
					size="small"
					:columns="[
						{
							title: '选中',
...
export default defineComponent({
	...
	methods: {
		...
		onOptionChecked(index) {
			let options = this.formState.options;
